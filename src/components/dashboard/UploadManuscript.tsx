...
export default function UploadManuscript() {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      setIsUploading(true);
      const response = await paperService.upload({
        file,
    ...
    <div className="bg-theme-bg-card rounded-xl border border-theme-border-main p-5">
      <div className="flex items-start gap-3 mb-4">
        ...
        >
          <CloudUpload
            size={18}
            style={{ color: "var(--theme-accent-main)" }}
          />
        </div>
        <div>
          <h2
          ...
          <p className="text-xs text-theme-text-light leading-relaxed mt-0.5 max-w-md">
            Upload your manuscript to run a comprehensive audit on research
            integrity, publication quality, and submission readiness.
          </p>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div
          ...
          className={`border-2 border-dashed rounded-xl px-6 py-8 flex flex-col items-center justify-center text-center transition-all ${
            dragOver
              ? "border-theme-accent-secondary bg-theme-bg-hover"
              : "border-theme-border-secondary hover:border-theme-accent-secondary hover:bg-theme-bg-hover/50"
          }`}
        >
          <Upload
            size={28}
            className="mb-3"
            style={{ color: "var(--theme-accent-upload)" }}
          />
          ...
          <p className="text-[11px] text-theme-text-light mt-3">
            Supports PDF, DOCX | Max size: 100MB
          </p>
        </div>
          ...
          {AI_FEATURES.map((f) => (
            <div key={f.title} className="flex gap-3 items-start">
              ...
              <div>
                ...
                <p className="text-[11px] text-theme-text-light mt-0.5 leading-relaxed">
                  {f.desc}
                </p>
              </div>
            </div>
